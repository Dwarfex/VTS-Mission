...
vts_respawn_camera_on=
{
	if !(isnil "vts_respawn_camera") then 
	{
		vts_respawn_camera cameraeffect ["terminate","back"];
		camdestroy vts_respawn_camera;	
	};
	private ["_cam","_pos"];
	_pos=getpos player;
	vts_respawn_camera = "camera" camcreate [_pos select 0,_pos select 1,(_pos select 2)+2];
	vts_respawn_camera attachto [player,[0,0,3]];
	vts_respawn_camera camSetTarget player;
	vts_respawn_camera cameraeffect ["internal","back"];
	vts_respawn_camera camcommit 0 ; 
	showCinemaBorder  false;	
	cameraEffectEnableHUD true;	
};

vts_respawn_camera_off=
{
	if (isnil "vts_respawn_camera") exitwith {};
	vts_respawn_camera cameraeffect ["terminate","back"];
	camdestroy vts_respawn_camera;
	vts_respawn_camera=nil;
	closedialog 8010;
};

vts_respawn_showrespawndialog=
{
	private ["_loop"];
	createdialog "vts_respawn_ui";
	_loop=true;
	while {_loop} do
	{
		waitUntil {sleep 0.1;isnull (finddisplay 8010)};
		sleep 2;
...
vts_respawn_forcerespawn_uibutton=
{
	if ([player] call vts_respawn_canrespawn) then
	{
		[] call vts_respawn_forcerespawnaction;
	};
	closedialog 8010;
};
...
vts_respawn_norevive=
{
	...
	if (pa_revivetype>0)  then
	{
		...
		else
		{
			...
			//Disable TFR when dead
			if !(isnil "TFAR_fnc_forceSpectator") then
			{
				[_unit,true] spawn TFAR_fnc_forceSpectator;
			};
			...
			if !(isnil "TFAR_fnc_forceSpectator") then
			{
				[_unit,false] spawn TFAR_fnc_forceSpectator;
			};
...
vts_respawn_waitingforrevive=
{
	...
	//Activated camera to lock control
	[] call vts_respawn_camera_on;
	...
	if ((pa_revivetype!=4) && (pa_revivetype!=3)) then
	{
		...
		[] spawn vts_respawn_showrespawndialog;
	};
	...
	//Disable revive camera
	[] call vts_respawn_camera_off;
