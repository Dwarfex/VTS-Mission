...
if ((_hasDriver == 1) && (isNull (driver _vehicle))) then
{
	if (!_dummy) then
	{
		...
		[_unit] spawn vts_DamageSecurityTempo;
	}
	else
	{
		_crew = _crew + [1];
...
//Fix commander issue not assigned as commander (Making targeting acquisition issue, coz the command is assigned gunner on a commander turret...)
