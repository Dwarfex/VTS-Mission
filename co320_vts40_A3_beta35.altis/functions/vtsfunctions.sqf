...
vts_fastropedraw=
{
	private ['_rope','_ropeY','_vecdir','_gen','_caller','_id','_initpos','_simu','_loop','_vx','_vy','_step','_time','_source','_possource','_poslink','_offset','_posoffset','_ropedist','_maxropedist','_dist'];
	_source=_this select 0;
	_gen=_this select 1;
	...
	{
		waituntil {(vehicle _gen==_gen)};
		_rope=ropeCreate [_source,[0,0,0],3];
		_gen disableCollisionWith _rope;
		[_gen,[0,0.35,-0.5],[0,0,0]] ropeAttachTo _rope;
		ropeUnwind [_rope,7,_maxropedist,true];
	};
	...
	if (local _source) then
	{
		_gen ropedetach _rope;
		ropeUnwind [_rope,4,(vts_FastRopeLength*-1),true];
		[_rope] spawn
		{
			waituntil {sleep 0.1;ropeUnwound (_this select 0)};
...
vts_fastropego=
{
	private ['_rope','_gen','_caller','_id','_initpos','_simu','_loop','_vx','_vy','_step','_time','_source','_possource','_poslink','_maxropedist'];
	...
	//[_source,_gen] spawn vts_fastropedraw;
	...
	*/
	...
	{_gen enableCollisionWith _x;} foreach playableUnits;
...
{
	private ['_rope','_gen','_caller','_id','_initpos','_simu','_loop','_vx','_vy','_step','_time','_source','_possource','_poslink'];
	_source=_this select 0;
	_gen=_this select 1;
	...
	_simu=0.01;
	_time=time;
	//_rope=ropeCreate [_source, [0,0,0.5], _gen, [0,0,1],_source distance _gen];
	while {_loop} do
	{		
		_possource=visiblePositionASL _source;
		if !(surfaceiswater _possource) then {_possource=asltoatl _possource;};
...
{
	private ['_rope','_attachoffset','_properrelease','_lifterror','_target','_objs','_obj','_pos','_veh','_loop','_speed','_dir','_v1','_v2','_mod','_n','_height','_targetbbox','_targetposasl','_vehposasl','_targettop','_targetbottom','_posfix','_hookstilldown','_playervehbbox'];
	...
	if !(isnull _target) then
	{
		...
		if !(_veh setSlingLoad _target) then
		{
			_rope=ropeCreate [_veh, [0,0,0.5], _target, [0,0,1],_veh distance _target];	
		};
		...
		while {_loop} do
		{
			...
			if (terrainIntersectASL [_vehposasl,[_targetposasl select 0,_targetposasl select 1,(_targetposasl select 2)+(_targetbottom+_targettop)-(_posfix select 2)]]) then
			{
				_hookstilldown=true;
				_loop=false;
				player sidechat str "terrain inter";
			};
...
{
	private ['_rope','_var','_veh','_lastpos','_curpos'];
	_veh=_this select 0;
	_rope=ropeCreate [_veh,[0,0,0.5],15];
...
vts_helicopterlifthookdowned=
{
	...
	[_veh] spawn vts_helicopterliftdrawnhookdowned;
	while {_loop} do
	{
