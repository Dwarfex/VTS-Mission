...
{
	private ["_var","_frame","_data","_markers"];
...
{
	//[] call vts_perfstart;
	private ["_array","_type","_pos","_name","_dir","_side","_engage","_typeof"];
	_array=[];
	{
		if (vehicle _x==_x) then
		{
			...
			_array set [count _array,[_name,_type,_pos,_dir,_engage]];
		};
...
vts_aar_drawelemet=
{
	...
	_name=_frame select 0;
	_type=_frame select 1;
	_pos=_frame select 2;
	_dir=_frame select 3;
	_engage=_frame select 4;
...
vts_aar_drawframe=
{
	private ["_frame","_marker","_markersarray","_i","_element"];
	_frame=_this select 0;
	_markersarray=[];
	for "_i" from 0 to (count _frame)-1 do
	{	
		_element=_frame select _i;
		_marker=[_element] call vts_aar_drawelemet;
...
vts_aar_open=
{
	...
	((finddisplay 8008) displayctrl 1611) ctrlAddEventHandler  ["MouseButtonDown","['Importing : Please wait, it may take a few minutes'] call vts_aar_message;"];
	((finddisplay 8008) displayctrl 1610) ctrlAddEventHandler  ["MouseButtonDown","['Exporting : Please wait, it may take a few minutes'] call vts_aar_message;"];
...
vts_aar_export=
{
	private ["_filename","_i","_numframes","_l","_elements","_strnum","_elementsize","_curelements","_curitem"];
	...
	{		
		_strnum=(str _i);
		call compile ("_elements=count vts_aar_frame_"+_strnum+";");
		_pnum=0;
		_curdbsize=0;
		_dbelement=0;
		_curelements=[];
		...
		while {_loop} do
		{
			...
			call compile ("_curitem=vts_aar_frame_"+_strnum+" select _pnum;");
			if ((_curitem select 0)!="") then {_curdbsize=_curdbsize+64;};
			...
			_curelements set [count _curelements,_curitem];
			_pnum=_pnum+1;
		...
		_strnum=(str _i);
		call compile ("_elements=count vts_aar_frame_"+_strnum+";");
		[_filename,("aar_frame_"+_strnum),"vts_aar_frame_elements",_elements] call iniDB_write;
		for "_l" from 0 to (_elements-1) do
		{
			call compile ("[_filename,'aar_frame_"+_strnum+"','vts_aar_element_"+(str _l)+"',(vts_aar_frame_"+_strnum+") select "+(str _l)+"] call iniDB_write;");
		};*/
	};
...
vts_aar_import=
{
	private ["_filename","_numframes","_i","_worldname","_elements","_strnum","_l","_buildframe"];
	if (isnil "iniDB_version") exitwith 
	{
