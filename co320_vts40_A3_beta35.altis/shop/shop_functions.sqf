...
vts_shoploadoutsave=
{
	...
	if (vts_shoploadouttype==2) then
	{
		...
		{
			_refresh=false;
			_txt="Error : You must be connectd as GM and server ADMIN to modify Server loadout";
		};
	};
    hint _txt;
    [_txt] call vts_shopdisplaymessage;   
...
vts_shopbuy={
  ...
  else
  {
	...
	if (vtsarmaversion>2) then
	{
		...
		{
			//Gears
			case 801: {call compile "_pocketitems=uniformItems player;removeuniform player;player forceAddUniform _buyitemname;{player addItemToUniform _x} foreach _pocketitems;_additem=true;";};
		...
		if (_buytitemtype==131072 or _buytitemtype==616 or _buytitemtype==619 or _buytitemtype==401 or _buytitemtype==301 or _buytitemtype==620 or _buytitemtype==201 or _buytitemtype==101 or _buytitemtype==621 or _buytitemtype==4096) then
		{
