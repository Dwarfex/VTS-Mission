...
parachutealtitude = _this select 0;
publicvariable "parachutealtitude";
    ...
    spawn_x = _pos select 0;
     ...
     ";
        ...
        //***** Code come here *********
	      ...
	      _list = nearestObjects  [[spawn_x,spawn_y,0],["CAManBase","Car","Truck","Tank","Helicopter","Plane","Boat"],500];
	      ...
	      if ((count _list)==0) exitwith {breakclic = 0;hint "!!! No units found !!!"};
			...
			_groupunit=[];
			//Empty vehicle or body ?
	      if (isnull (group _source)) then 
			{
				_groupunit=[_source];
			}
			//Else a valid group to TP
		  else
			{
				_groupunit = units group _source;
			};
	      _count=0;
	      {
          _count=_count+1;
	      ...
	      [_groupunit] execVM "Computer\console\parachuteagroup.sqf";
