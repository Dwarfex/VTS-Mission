...
_distance = radius;
//_ramdomdist = round(random _distance / 3);
_angle = random 360;
_way = round(random 1);
...
while {_n<=_nmax} do
  ...
  _wp = _group addWaypoint [[_sav_positX,_sav_positY,0], 2];
  ...
  _wp setWaypointType "CYCLE";
  	...
  	call compile format["
	_marker = createMarkerLocal [""WaypMark%1"",[_sav_positX,_sav_positY,0]];
	_marker setMarkerTextLocal ""WP %1"";
	_marker setMarkerTypeLocal ""mil_Dot"";
	_marker setMarkerSizeLocal [1, 1];
	_marker setMarkerDirLocal 0;
...
//_wp = _group addWaypoint [[_sav_positX,_sav_positY,0], 0];
//_wp = _group addWaypoint [[_sav_positX,_sav_positY,0], 2];

sleep 3;
for "_n" from 0 to _nmax+1 do
{
